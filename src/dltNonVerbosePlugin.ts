...
export class DltNonVerbosePlugin extends DltTransformationPlugin {
    ...
    constructor(uri: vscode.Uri, public treeViewNode: TreeViewNode, treeEventEmitter: vscode.EventEmitter<TreeViewNode | null>, options: any) {
        ...
        // open xml files...
        if (this.enabled) {
            const ownFibexDir = 'fibexDir' in options && typeof options['fibexDir'] === 'string' && options['fibexDir'].length ? options['fibexDir'] : undefined;
            if (ownFibexDir) {
        ...
        this._servicesNode = { id: createUniqueId(), label: `Frames (${idFrames.length}/${FibexLoader.framesWithKey.size})`, children: [], parent: treeViewNode, tooltip: '', uri: uri };
        treeViewNode.children.push(this._servicesNode);
        // add frames:
        idFrames.forEach(id => {
            const frame = FibexLoader.framesWithKey.get(id);
            if (frame) {
                const pduTexts: string[] =
                    frame.pdus.map(p => `${p.desc && p.desc.length > 0 ? `'${p.desc}'` : p.signalRefs?.map(s => `<${s}>`).join(' ')}`);
    ...
    matches(msg: DltMsg): boolean {
        ...
        if (msg.verbose) { return false; }
        if (msg.mstp === MSTP.TYPE_CONTROL) { return false; } // todo later on TYPE_INFO... is still assumed

        // do we know the frame?
        // here the _payloadData still exists... todo refactor this. that's not nice.
        const id = msg._payloadData.length >= 4 ? `ID_${msg.isBigEndian ? msg._payloadData.readUInt32BE(0) : msg._payloadData.readUInt32LE(0)}` : undefined;
        if (id !== undefined) {
                        ...
                        if (buf.length !== frame.byteLength) {
                            ...
                            if (manufExt) {
                                ...
                                switch (manufExt.MESSAGE_TYPE) {
                                    case 'DLT_TYPE_LOG': msg.mstp = MSTP.TYPE_LOG; break;
                                    case 'DLT_TYPE_APP_TRACE': msg.mstp = MSTP.TYPE_APP_TRACE; break;
                                    case 'DLT_TYPE_NW_TRACE': msg.mstp = MSTP.TYPE_NW_TRACE; break;
                                    case 'DLT_TYPE_CONTROL': msg.mstp = MSTP.TYPE_CONTROL; break;
                                    default:
                                        break;
                                }
                                ...
                                let doSet = false;
                                if (apid && msgApid.length === 0) { doSet = true; }
                                if (ctid && msgCtid.length === 0) { doSet = true; }
                                if (doSet) {
                                ...
                                if (msg.mstp === MSTP.TYPE_LOG) {
                                    switch (level) {
                                        case 'DLT_LOG_INFO':
                                            msg.mtin = MTIN_LOG.LOG_INFO; break;
                                        case 'DLT_LOG_ERROR':
                                            msg.mtin = MTIN_LOG.LOG_ERROR; break;
                                        case 'DLT_LOG_WARN':
                                            msg.mtin = MTIN_LOG.LOG_WARN; break;
                                        case 'DLT_LOG_FATAL':
                                            msg.mtin = MTIN_LOG.LOG_FATAL; break;
                                        case 'DLT_LOG_DEBUG':
                                            msg.mtin = MTIN_LOG.LOG_DEBUG; break;
                                        case 'DLT_LOG_VERBOSE':
                                            msg.mtin = MTIN_LOG.LOG_VERBOSE; break;
                            ...
                            if (bytesProcessed !== frame.byteLength) {
                                console.warn(`DltNonVerbosePlugin frame ${frame.id} '${frame.shortName} 'processed only ${bytesProcessed}/${frame.byteLength} bytes payload!`);
                            } else {
