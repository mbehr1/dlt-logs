...
export async function askSingleTime(refTime: Date, maxTime: Date | undefined): Promise<Date> {
    ...
    const timeInitialValue = (): string => {
        const v = `${refTime.getHours().toString().padStart(2, '0')}:${refTime.getMinutes().toString().padStart(2, '0')}:${refTime.getSeconds().toString().padStart(2, '0')}`;
        return v;
    };
    const timeRestrictPIs: PickItem[] = [new PickItem('')];
    const timeRestrictItems = (): PickItem[] => {
        const val = timeInitialValue();
        timeUpdateHintOrError(val,
            refTime,
            val.length === 0 ? `enter time like: '[+xd]hh:mm[:ss]' (xd as additional days (e.g. +2d)).` : ''
        );
    ...
    const timeIsValid = (v: string, storeValue: boolean = false): boolean => {
        ...
        if (!rTest) { timeUpdateHintOrError('', undefined, `not matching '[+xd]hh:mm[:ss]'`, true); return false; }
        // parse the times:
        const timeR = timeRestrictRegExp.exec(v);
        if (timeR !== null) {
            const gotDays = timeR[1] !== undefined;
            const gotTimeFrom = timeR[2] !== undefined;
            if (!gotTimeFrom) { timeUpdateHintOrError('', undefined, `invalid time`, true); return false; }
    ...
    let toRet = new Promise<Date>((resolve, reject) => {
        let stepInput = new MultiStepInput(`Enter `, [
            { title: `time [${refTime.toLocaleTimeString()}${maxTime ? `- ${maxTime.toLocaleString()}` : `...`}]`, initialValue: timeInitialValue, items: timeRestrictItems, isValid: timeIsValid, onValue: timeOnValue, onMoreItems: timeOnMoreItems, canSelectMany: false }
        ], { canSelectMany: false });
        stepInput.run().then(async () => {
            if (results.time !== undefined) { resolve(results.time); } else { reject(`no time entered`); }
