...
export function editFilter(doc: FilterableDocument & ReportDocument, newFilter: DltFilter, optArgs?: { payload?: string, isAdd?: boolean }) {
    return new Promise<boolean>((resolveEdit) => {
        ...
        try { // colors is an objecct with e.g. "blue":[0,0,255]
        ...
        let stepInput = new MultiStepInput(`${isAdd ? 'add' : 'edit'} filter...`, [
            ...
            { title: `choose marker colour`, items: colorItems, initialValue: () => { return typeof newFilter.filterColour === 'object' ? JSON.stringify(newFilter.filterColour) : newFilter.filterColour; }, onValue: (v) => { /* can parse to Object? */let o = undefined; try { o = JSON.parse(v); if (typeof (o) !== 'object') { o = undefined; } } catch (e) { }; newFilter.filterColour = o !== undefined ? o : (v.length ? v : "blue"); }, isValid: (v => { return (colors as any)[v] !== undefined; }), skipStep: () => newFilter.type !== DltFilterType.MARKER }, // todo add hex codes support and proper support for filterColour as object!
