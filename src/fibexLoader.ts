...
export class FibexLoader {
  ...
  private static parseFibexFrames(xmlFrames: any[]) {
    ...
    for (let i = 0; i < xmlFrames.length; ++i) {
      ...
      try {
        ...
        const pdus =
          xmlFrame?.['fx:PDU-INSTANCES']?.['fx:PDU-INSTANCE']?.map((xPdu: any) => {
      ...
      try {
        ...
        if ('fx:COMPLEX-DATATYPE-CLASS' in datatypeObj) {
          switch (datatypeObj['fx:COMPLEX-DATATYPE-CLASS']) {
            ...
            default:
              console.warn(
                `FibexLoader.parseFibexDatatypes unknown complex-datatype-class:'${
                  datatypeObj['fx:COMPLEX-DATATYPE-CLASS']
                }: ${JSON.stringify(datatypeObj, undefined, 2)}`,
              )
              break
          }
        }
        ...
        if ('fx:ENUMERATION-ELEMENTS' in datatypeObj) {
          ...
          if (Array.isArray(enumsObj)) {
            ...
            enums = new Map(
              enumsObj.map((e) => [
                e['fx:VALUE'],
                { synonym: e['fx:SYNONYM'] || `SYNONYM missing for ${e['fx:VALUE']}`, desc: e['fx:DESC'] },
              ]),
            )
          } else {
    ...
    if (paramsArr.length) {
      ...
      try {
        for (let i = 0; i < paramsArr.length; ++i) {
          ...
          if (newParam.pos && newParam.pos >= 0) {
            needsSort = true
          }
          toRet.push(newParam)
        }
    ...
    if (serviceMethods !== undefined) {
      ...
      for (let i = 0; i < methodsArray.length; ++i) {
        ...
        try {
          ...
          FibexLoader.addMethod(methods, {
            mid: Number(serviceMethod['service:METHOD-IDENTIFIER']),
            shortName: serviceMethod['ho:SHORT-NAME'],
            desc: serviceMethod['ho:DESC'],
    ...
    if (serviceFields !== undefined) {
      ...
      if (Array.isArray(fieldsArray)) {
        if (!fieldsArray.length) {
          console.warn(`FibexLoader.parseFibexFields got no fields from ${JSON.stringify(serviceFields)} `)
        }
          ...
          try {
            ...
            if ('service:GETTER' in fieldObj) {
              ...
              FibexLoader.addMethod(methods, {
                ...
                array: arrayInfo,
                mid: Number(serviceMethod['service:METHOD-IDENTIFIER']),
                shortName: `get_${shortName}_field`,
                desc: desc,
                fieldName: shortName,
                inputParams: inputParams,
                returnParams: returnParams,
            ...
            if ('service:SETTER' in fieldObj) {
              ...
              FibexLoader.addMethod(methods, {
                ...
                array: arrayInfo,
                mid: Number(serviceMethod['service:METHOD-IDENTIFIER']),
                shortName: `set_${shortName}_field`,
                desc: desc,
                fieldName: shortName,
                inputParams: inputParams,
                returnParams: returnParams,
            ...
            if ('service:NOTIFIER' in fieldObj) {
              ...
              FibexLoader.addMethod(methods, {
                datatype: datatype,
                array: arrayInfo,
                /*orig: fieldObj,*/ mid: Number(serviceMethod['service:NOTIFICATION-IDENTIFIER']),
                shortName: `changed_${shortName}_field`,
                desc: desc,
                fieldName: shortName,
                inputParams: inputParams,
                returnParams: returnParams,
  ...
  static get loadedFibex() {
    return FibexLoader._loadedFibex
  }
  static get codings() {
    return FibexLoader._codings
  }
  static get datatypes() {
    return FibexLoader._datatypes
  }
  static get services() {
    return FibexLoader._services
  }
  static get frames() {
    return FibexLoader._frames
  }
  static get framesWithKey() {
    return FibexLoader._framesWithKey
  }
}
