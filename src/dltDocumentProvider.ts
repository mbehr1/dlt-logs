...
export class DltDocumentProvider implements vscode.FileSystemProvider, vscode.DocumentSymbolProvider, vscode.Disposable {
  ...
  constructor(
              ...
              if (ev.selections[0].isSingleLine) {
                ...
                if (time) {
                  if (this._autoTimeSync) {
                    // post time update...
                    console.log(
                      ` dlt-log posting time update ${time.toLocaleTimeString()}.${String(time.valueOf() % 1000).padStart(3, '0')}`,
                    )
    ...
    this._subscriptions.push(
      vscode.commands.registerTextEditorCommand('dlt-logs.sendTime', async (textEditor) => {
    ...
    this._subscriptions.push(
      vscode.commands.registerCommand('dlt-logs.toggleTimeSync', () => {
        console.log(`dlt-log.toggleTimeSync called...`)
        this._autoTimeSync = !this._autoTimeSync
        vscode.window.showInformationMessage(
          `Auto time-sync turned ${
            this._autoTimeSync
              ? 'on. Selecting a line will send the corresponding time.'
              : "off. To send the time use the context menu 'send selected time' command."
          }`,
        )
      }),
    )
    ...
    this._subscriptions.push(
      vscode.commands.registerTextEditorCommand('dlt-logs.sendTimeSyncEvents', async (textEditor) => {
        let data = this._documents.get(textEditor.document.uri.toString())
        if (data) {
    ...
    this._subscriptions.push(
      vscode.commands.registerTextEditorCommand('dlt-logs.adjustTime', async (textEditor) => {
        ...
        if (doc) {
          ...
          if (!doc.gotTimeSyncEvents && doc.lastSelectedTimeEv) {
            ...
            if (textEditor.selections.length === 1) {
              ...
              if (time) {
                ...
                let response: string | undefined = await vscode.window.showInformationMessage(
                  `Adjust based on last received time event (adjust by ${selTimeAdjustValue / 1000} secs)?`,
                  { modal: true },
                  'yes',
                  'no',
                )
          ...
          if (doManualPrompt) {
            vscode.window
              .showInputBox({
                prompt: `Enter new time adjust in secs (cur = ${curAdjustMs / 1000}):`,
                value: (curAdjustMs / 1000).toString(),
              })
  ...
  private async onTreeViewSave(node: TreeViewNode) {
    ...
    if (node instanceof DltFileTransfer) {
      ...
      if (fileTransfer && fileTransfer.isComplete) {
        let newFileUri = fileTransfer.uri.with({
          scheme: 'file',
          path: path.join(path.dirname(fileTransfer.uri.fsPath), fileTransfer.fileName),
        })
        return vscode.window.showSaveDialog({ defaultUri: newFileUri, filters: { all: ['*'] }, saveLabel: 'Save file as' }).then(
  ...
  public onTreeNodeCommand(command: string, node: TreeViewNode) {
    switch (command) {
      ...
      case 'setPosFilter':
        ...
        break
      case 'save':
        this.onTreeViewSave(node)
        break
      default:
        console.error(`dlt.onTreeNodeCommand unknown command '${command}'`)
        break
    }
  }
  ...
  provideDocumentSymbols(doc: vscode.TextDocument): vscode.ProviderResult<vscode.SymbolInformation[] | vscode.DocumentSymbol[]> {
    ...
    if (!dltDoc) {
      ...
      try {
        ...
        let ecus: vscode.DocumentSymbol = new vscode.DocumentSymbol(
          'ECUs',
          'Detected ECUs within that dlt-log',
          vscode.SymbolKind.Null,
          new vscode.Range(0, 0, 0, 0),
          new vscode.Range(0, 0, 0, 0),
        )
        lifecycleInfos.forEach((lcInfos, strEcu) => {
          let ecu: vscode.DocumentSymbol = new vscode.DocumentSymbol(
            strEcu,
            `Lifecycles for ${strEcu}`,
            vscode.SymbolKind.Enum,
            new vscode.Range(0, 0, 0, 0),
            new vscode.Range(0, 0, 0, 0),
          ) // todo could use first and last line here
          ...
          lcInfos.forEach((lcInfo) => {
            ...
            let lc: vscode.DocumentSymbol = new vscode.DocumentSymbol(
              `${lcInfo.lifecycleStart.toTimeString()}-${lcInfo.lifecycleEnd.toTimeString()}}`,
              `${lcInfo.logMessages.length} msgs, start index=${lcInfo.startIndex}`,
        ...
        let lifecycles: vscode.DocumentSymbol = new vscode.DocumentSymbol(
          'lifecycle 1',
          'detail bla fooo',
          vscode.SymbolKind.Event,
          new vscode.Range(5, 0, 20, 0),
  ...
  handleDidChangeSelectedTime(ev: SelectedTimeData) {
    this._documents.forEach((doc) => {
      if (doc.uri.toString() !== ev.uri.toString()) {
        // avoid reacting on our own events...
        ...
        if (ev.time.valueOf() > 0) {
          ...
          doc.lineCloseToDate(ev.time).then((line) => {
            if (line >= 0 && doc.textEditors.length > 0) {
              ...
              if (line >= 0) {
                console.log(
                  `dlt-log.handleDidChangeSelectedTime got no textEditors (${doc.textEditors.length}) for reveal of line ${line}. hidden?`,
                )
              }
            }
          })
        ...
        if (ev.timeSyncs?.length && doc.timeSyncs.length) {
          ...
          for (let i = 0; i < ev.timeSyncs.length; ++i) {
            ...
            for (let j = 0; j < doc.timeSyncs.length; ++j) {
              ...
              if (remoteSyncEv.id === localSyncEv.id) {
                console.log(
                  `  got id='${remoteSyncEv.id}' match. Checking value='${remoteSyncEv.value} at ${remoteSyncEv.time.toLocaleTimeString()}`,
                )
                if (remoteSyncEv.value === localSyncEv.value) {
                  console.log(
                    `   got id='${remoteSyncEv.id}',prio=${remoteSyncEv.prio} and value='${
                      remoteSyncEv.value
                    } match at ${remoteSyncEv.time.toLocaleTimeString()}, prio=${localSyncEv.prio}`,
                  )
          ...
          } else if (reBroadcastEvents.length) {
