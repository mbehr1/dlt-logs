...
export function generateRegex(toMatchArr: string[]): RegExp[] {
  ...
  if (toMatchArrCopy.length > 1) {
    ...
    while (startIndex < minStrLength) {
      ...
      if (prefixLen + startIndex >= minStrLength) {
        break
      }
      if (prefix.includes(' ')) {
      ...
      toMatchArrCopy = toMatchArrCopy.map(
        (toMatch) => toMatch.slice(0, startIndex + prefixLen) + toMatch.slice(startIndex + prefixLen).replace(regExWordFind, replWord),
      )
