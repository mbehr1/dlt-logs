...
export class DltDocument {
  ...
  get timeAdjustMs(): number {
    return this._timeAdjustMs
  } // read only. use adustTime to change
  ...
  constructor(
    ...
    this.treeNode = {
      id: util.createUniqueId(),
      label: `${path.basename(this._fileUri.fsPath)}`,
      uri: this.uri,
      parent: null,
      children: [this.lifecycleTreeNode, this.filterTreeNode, this.configTreeNode, this.pluginTreeNode],
      tooltip: undefined,
  ...
  stat(): vscode.FileStat {
    return {
      ...
      ctime: this._realStat.ctime.valueOf(),
      mtime: this._realStat.mtime.valueOf(),
      type: this._realStat.isDirectory()
        ? vscode.FileType.Directory
        : this._realStat.isFile()
  ...
  getDecorationFor(filter: DltFilter): vscode.TextEditorDecorationType | undefined {
    ...
    if (filter.decorationId) {
      return this._decorationTypes.get(filter.decorationId)
    }
  ...
  parseFilterConfigs(filterObjs: Object[] | undefined) {
    console.log(
      `DltDocument.parseFilterConfigs: have ${filterObjs?.length} filters to parse. Currently have ${this.allFilters.length} filters...`,
    )
    if (filterObjs) {
      let skipped = 0
      for (let i = 0; i < filterObjs.length; ++i) {
        try {
          ...
          if (containedIdx < 0) {
            ...
            console.log(
              ` DltDocument.parseFilterConfigs adding filter: name='${newFilter.name}' type=${newFilter.type}, enabled=${newFilter.enabled}, atLoadTime=${newFilter.atLoadTime}`,
            )
          } else {
            ...
            if ('type' in filterConf && 'id' in filterConf) {
              ...
              console.warn(
                `DltDocument skipped update of existingFilter=${existingFilter.id} due to wrong config: '${JSON.stringify(filterConf)}'`,
              )
            }
      ...
      for (let i = filterObjs.length - skipped; i < this.allFilters.length; ++i) {
        ...
        if (existingFilter.constructor === DltFilter) {
          ...
          console.log(
            ` DltDocument.parseFilterConfigs deleting existingFilter: name '${existingFilter.name}' ${
              existingFilter instanceof DltFileTransferPlugin
            } ${existingFilter instanceof DltFilter} ${existingFilter.constructor === DltFileTransferPlugin} ${
              existingFilter.constructor === DltFilter
            }`,
          )
  ...
  adjustTime(relOffset: number) {
    ...
    this.lifecycles.forEach((lcInfos) => {
      lcInfos.forEach((lcInfo) => {
        lcInfo.adjustTimeMs = this._timeAdjustMs
      })
    })
  ...
  static getFilter(allFilters: readonly DltFilter[], enabled: boolean, atLoadTime: boolean, negBeforePos: boolean = false) {
    let toRet: DltFilter[][] = []
    for (let i = DltFilterType.POSITIVE; i <= DltFilterType.EVENT + 1; ++i) {
      // +1 for NEGATIVE_BEFORE_POSITIVE
      toRet.push([])
    }
    ...
    try {
      ...
      for (let i = 0; i < nrMsgs; ++i) {
        ...
        if (foundAfterNegFilters) {
          ...
          if (matchingMsgs.length >= maxMsgsToReturn) {
            break
          }
        }
      }
  ...
  async applyFilter(
    ...
    try {
      ...
      for (let i = 0; i < nrMsgs; ++i) {
        if (i % 1000 === 0) {
          // provide process and responsiveness for UI:
        ...
        if (foundAfterNegFilters) {
          ...
          if (msg.index > this._maxFilteredMsgIndex) {
            this._maxFilteredMsgIndex = msg.index
          }
          ...
          if (this.decWarning && msg.mstp === MSTP.TYPE_LOG && msg.mtin === MTIN_LOG.LOG_WARN) {
            msg.decorations.push([
              this.decWarning,
              [
                {
                  range: new vscode.Range(this.filteredMsgs.length - 1, 0, this.filteredMsgs.length - 1, 21),
                  hoverMessage: new vscode.MarkdownString('$(warning) LOG_WARN', true),
                },
              ],
            ])
            gotDeco = true
          ...
          if (!gotDeco && this.decError && msg.mstp === MSTP.TYPE_LOG && msg.mtin === MTIN_LOG.LOG_ERROR) {
            msg.decorations.push([
              ...
              [
                {
                  range: new vscode.Range(this.filteredMsgs.length - 1, 0, this.filteredMsgs.length - 1, 21),
                  hoverMessage: new vscode.MarkdownString('$(error) LOG_ERROR', true),
                },
              ],
            ])
          ...
          if (!gotDeco && this.decFatal && msg.mstp === MSTP.TYPE_LOG && msg.mtin === MTIN_LOG.LOG_FATAL) {
            msg.decorations.push([
              this.decFatal,
              [{ range: new vscode.Range(this.filteredMsgs.length - 1, 0, this.filteredMsgs.length - 1, 21), hoverMessage: `LOG_FATAL` }],
            ])
            // todo see above
          }
                ...
                if (decType) {
                  msg.decorations.push([
                    ...
                    [
                      {
                        range: new vscode.Range(this.filteredMsgs.length - 1, 0, this.filteredMsgs.length - 1, 21),
                        hoverMessage: `MARKER ${decFilter.name}`,
                      },
                    ],
                  ])
                  gotDeco = true
      ...
      if (lcDecos[0] !== undefined) {
        this._allDecorations.set(lcDecos[0], [])
      }
      if (lcDecos[1] !== undefined) {
        this._allDecorations.set(lcDecos[1], [])
      }
      ...
      if (lcDecos[0] !== undefined || lcDecos[1] !== undefined) {
        ...
        if (maxEcu) {
          // todo this has a bug if lifecycles are filtered...
            ...
            for (let lcCnt = 0; lcCnt < lcs.length; ++lcCnt) {
              ...
              if (lcDec !== undefined) {
                ...
                const dec = {
                  ...
                  hoverMessage: `lifecycle: ${lc.lifecycleStart.toLocaleTimeString()}-${lc.lifecycleEnd.toLocaleTimeString()}`,
                }
  ...
  updateStatusBarItem(item: vscode.StatusBarItem) {
    ...
    item.tooltip = `DLT: ${this._fileUri.fsPath}, showing max ${this._maxNrMsgs} msgs, ${this._timeAdjustMs / 1000}s time-adjust, ${
      this.timeSyncs.length
  ...
  msgByLine(line: number): DltMsg | undefined {
    const msgs = this.filteredMsgs ? this.filteredMsgs : this.msgs
    // line = index
    if (line >= this.staticLinesAbove.length && line < this._skipMsgs + msgs.length) {
      // we don't have to check whether the msg is visible here
  ...
  public provideHover(position: vscode.Position): vscode.ProviderResult<vscode.Hover> {
    ...
    if (posTime) {
      let mdString = new vscode.MarkdownString(
        util.escapeMarkdown(
          `${posTime.toLocaleTimeString()}.${String(posTime.valueOf() % 1000).padStart(3, '0')} index#=${msg.index} timestamp=${
            msg.timeStamp
          } reception time=${msg.timeAsDate.toLocaleTimeString()} mtin=${msg.mtin}`,
        ),
        true,
      )
      ...
      if (msg.lifecycle !== undefined) {
        ...
        if (apidInfos !== undefined) {
          ...
          if (ctidInfo !== undefined) {
            ctidDesc = `: ${util.escapeMarkdown(ctidInfo)}`
          }
        }
      }
      mdString.appendMarkdown(
        `| calculated time | ${util.escapeMarkdown(posTime.toLocaleTimeString())}.${String(posTime.valueOf() % 1000).padStart(
          ...
          `| reception time | ${util.escapeMarkdown(msg.timeAsDate.toLocaleTimeString())}.${String(
      ...
      return new vscode.Hover({
        language: 'dlt-log',
        value: `calculated time: <none> index#=${msg.index} timestamp=${
          msg.timeStamp
  ...
  notifyVisibleRange(range: vscode.Range) {
    ...
    if (triggerBelowLine - triggerAboveLine < 10) {
      console.log(
        ` notifyVisibleRange ignoring as range too small (visible: [${triggerAboveLine}-${triggerBelowLine}]) current: [${this._skipMsgs}-${
    ...
    if (this._renderTriggered) {
      console.log(
        ` notifyVisibleRange ignoring as render triggered (visible: [${triggerAboveLine}-${triggerBelowLine}]) current: [${
          this._skipMsgs
        }-${this._skipMsgs + this._maxNrMsgs})`,
      )
    }
    if (triggerAboveLine <= this._maxNrMsgs * 0.2) {
      ...
      if (this._skipMsgs > 0) {
        if (!this._renderTriggered) {
          console.log(
            ` notifyVisibleRange(visible: [${triggerAboveLine}-${triggerBelowLine}]) current: [${this._skipMsgs}-${
              this._skipMsgs + this._maxNrMsgs
            }) triggerAbove`,
          )
  ...
  revealByMsgsIndex(i: number): number {
    // msgs Index = the i inside msgs/filteredMsgs[i]
    ...
    if (this._skipMsgs <= i && this._skipMsgs + this._maxNrMsgs > i) {
      ...
      for (let newSkipMsgs = -this._maxNrMsgs; ; newSkipMsgs += this._maxNrMsgs / 2) {
        // todo might need special handling for upper bound!
  ...
  revealIndex(index: number): number {
    // return line nr that the msg with index will get:
    console.log(`revealIndex(${index})...`)
    if (this.filteredMsgs) {
      // todo renderTriggered needed?
      for (let i = 0; i < this.filteredMsgs.length; ++i) {
        if (this.filteredMsgs[i].index >= index) {
          // we want line msg i:
          return this.revealByMsgsIndex(i)
  ...
  async toggleSortOrder(): Promise<void> {
    ...
    if (this._sortOrderByTime) {
      this.msgs.sort((a, b) => {
        ...
        if ((a.lifecycle !== undefined && b.lifecycle === undefined) || (a.lifecycle === undefined && b.lifecycle !== undefined)) {
  ...
  async renderLines(
    skipMsgs: number,
    progress?: vscode.Progress<{ increment?: number | undefined; message?: string | undefined }>,
  ): Promise<void> {
      ...
      if (!this.decorations || this._skipMsgs !== skipMsgs) {
        ...
        this._allDecorations.forEach((decOpts, decType) => {
          ...
          for (let i = 0; i < decOpts.length; ++i) {
            ...
            if (
              (decOpt.range.start.line >= skipMsgs && decOpt.range.start.line < skipMsgs + this._maxNrMsgs) ||
              ...
              let newDecOpt: vscode.DecorationOptions = {
                renderOptions: decOpt.renderOptions,
                hoverMessage: decOpt.hoverMessage,
                range: new vscode.Range(newStartLine, decOpt.range.start.character, newEndLine, decOpt.range.end.character),
              }
    ...
    if (false && renderRangeStartLine > 0) {
      // todo someparts are off by 1 then
   ...
   * @returns
   */
  static async textLinesForMsgs(
    columns: ColumnConfig[],
    msgs: ViewableDltMsg[],
    msgIndexStart: number,
    msgIndexEndIncl: number,
    ...
    for (let c = 0; c < columns.length; ++c) {
      ...
      switch (column.name) {
        case 'index':
          showIndex = column.visible
          break
        case 'recorded time':
          showTime = column.visible
          break
        case 'calculated time':
          showCalculatedTime = column.visible
          break
        case 'timestamp':
          showTimestamp = column.visible
          break
        case 'ecu':
          showEcu = column.visible
          break
        case 'apid':
          showApid = column.visible
          break
        case 'ctid':
          showCtid = column.visible
          break
        case 'text':
          showPayload = column.visible
          break
        case 'mcnt':
          showMcnt = column.visible
          break
        case 'type':
          showType = column.visible
          break
        case 'subtype':
          showSubtype = column.visible
          break
        case 'mode':
          showMode = column.visible
          break
        default:
          {
    ...
    for (let j = msgIndexStart; j < msgIndexEnd; ++j) {
      ...
      try {
        if (showIndex) {
          toRet += String(msg.index).padStart(maxIndexLength) + ' '
        }
        if (showTime) {
        ...
        if (showCalculatedTime) {
          let timeC =
            msg.mstp === MSTP.TYPE_CONTROL && msg.mtin === MTIN_CTRL.CONTROL_REQUEST
              ? msg.receptionTimeInMs
              : msg.lifecycle
                ? msg.lifecycle.lifecycleStart.valueOf() + msg.timeStamp / 10
                : msg.receptionTimeInMs
          toRet += new Date(timeC).toLocaleTimeString() + ' '
        } // todo pad to len of max time? (23:59:59?)
        if (showTimestamp) {
          toRet += (msg.timeStamp / 10000).toFixed(4).padStart(9) + ' '
        }
        if (showMcnt) {
          toRet += String(msg.mcnt).padStart(3) + ' '
        }
        if (showEcu) {
          toRet += String(msg.ecu).padEnd(5)
        } // 5 as we need a space anyhow
        if (showApid) {
          toRet += String(msg.apid).padEnd(5)
        }
        if (showCtid) {
          toRet += String(msg.ctid).padEnd(5)
        }
        if (showType) {
          switch (msg.mstp) {
            case MSTP.TYPE_LOG:
        ...
        if (showMode) {
          toRet += msg.verbose ? 'verbose ' : 'non-verbose '
        }
        if (showPayload) {
          toRet += msg.payloadString
        }
        toRet += '\n'
      } catch (error) {
  ...
  async checkFileChanges() {
    ...
    if (stats.size > this._parsedFileLen) {
      ...
      if (this._parsedFileLen === 0 && stats.size > LOAD_TIME_ASSIST_TRIGGER_SIZE) {
        await vscode.window
          .showWarningMessage(
            `The file is quite large (${Number(stats.size / 1000000).toFixed(0)}MB). Do you want to check load time filters?`,
            { modal: true },
            'Check',
          )
          .then(async (value) => {
            if (value === 'Check') {
              await loadTimeFilterAssistant(this._fileUri, this.allFilters)
                .then((removedApids) => {
                  ...
                  if (Array.isArray(removedApids) && removedApids.length > 0) {
                    ...
                    for (let i = 0; i < removedApids.length; ++i) {
                      this.filterTreeNode.children.push(
                        new FilterNode(null, this.filterTreeNode, this.allFilters[this.allFilters.length - removedApids.length + i]),
                      )
                    }
                    this._treeEventEmitter.fire(this.filterTreeNode)
                  }
      ...
      return vscode.window
        .withProgress(
          { cancellable: false, location: vscode.ProgressLocation.Notification, title: 'loading dlt file...' },
          async (progress) => {
            // do we have any filters to apply at load time?
            const [posFilters, negFilters, decFilters, eventFilters, negBeforePosFilters] = DltDocument.getFilter(
              this.allFilters,
              true,
              true,
            )
            ...
            do {
              ...
              if (read) {
                ...
                const parseInfo = DltDocument.dltP.parseDltFromBuffer(
                  ...
                  0,
                  this.msgs,
                  { transformPlugins: this._transformationPlugins },
                  posFilters,
                  negFilters,
                  negBeforePosFilters,
                )
                ...
                if (read !== parseInfo[1]) {
                  ...
                  if (curTime[1] / 1000000 > 100) {
                    // 100ms passed
                    progress.report({
                      message: `processed ${Number((100 * this._parsedFileLen) / stats.size).toFixed(1)}%: ${Number(
                        this._parsedFileLen / 1000000,
                      ).toFixed(1)}/${Number(stats.size / 1000000).toFixed(1)}MB`,
                    })
                    await util.sleep(10) // 10ms each 100ms
                    startTime = process.hrtime()
                  }
  ...
  private updateLifecycleTreeNode() {
    ...
    this.lifecycles.forEach((lcInfo, ecu) => {
      ...
      let ecuNode: EcuNode = {
        id: util.createUniqueId(),
        swVersions: sw,
        label: `ECU: ${ecu}, SW${sw.length > 1 ? `(${sw.length}):` : `:`} ${sw.join(' and ')}`,
      ...
      // we do add one node with the APIDs/CTIDs:
      {
        ...
        lcInfo.forEach((l) => {
          l.apidInfos.forEach((v, k) => {
            ...
            if (apidInfo === undefined) {
              ...
              if (apidInfo.desc.length === 0 && v.desc.length > 0) {
                apidInfo.desc = v.desc
              }
            }
            v.ctids.forEach((desc, ctid) => {
              ...
              if (ctidInfo === undefined) {
                ...
                if (ctidInfo.desc.length === 0 && desc.length > 0) {
                  ctidInfo.desc = desc
                }
              }
            })
        ...
        const apidsNode = new DynFilterNode(
          `APIDs (${apidSet.size}) / CTIDs`,
          undefined,
          ecuNode,
          `symbol-misc`,
          {
        ...
        apidSet.forEach((info, apid) => {
          const apidNode = new DynFilterNode(
            `'${apid}'(${info.ctids.size})${info.desc.length ? `: ${info.desc}` : ''}`,
            `desc='${info.desc}', apid = 0x${Buffer.from(apid).toString('hex')}`,
            apidsNode,
            undefined,
            {
              ecu: ecuWoSh,
          ...
          info.ctids.forEach((ctidInfo, ctid) => {
            const ctidNode = new DynFilterNode(
              `'${ctid}'${ctidInfo.desc.length ? `: ${ctidInfo.desc} ` : ''}`,
              `desc='${ctidInfo.desc}', ctid = 0x${Buffer.from(ctid).toString('hex')}`,
              apidNode,
              undefined,
              {
                ecu: ecuWoSh,
      ...
      for (let i = 0; i < lcInfo.length; ++i) {
        const lc = lcInfo[i]
        ecuNode.children.push(
          new LifecycleNode(this.uri.with({ fragment: lc.startIndex.toString() }), ecuNode, this.lifecycleTreeNode, lc, i + 1),
        )
      }
    })
    this.autoEnableConfigs()
  ...
  onTreeViewDidChangeSelection(event: vscode.TreeViewSelectionChangeEvent<TreeViewNode>) {
    if (event.selection.length && event.selection[0].uri && event.selection[0].uri.fragment.length) {
      console.log(
        `dlt.onTreeViewDidChangeSelection(${event.selection.length} ${event.selection[0].uri} fragment='${
          event.selection[0].uri ? event.selection[0].uri.fragment : ''
        }')`,
      )
