...
export class DltRewriteMsgPlugin extends DltTransformationPlugin {
  ...
  constructor(
    ...
    this._rewrites = Array.isArray(options['rewrites'])
      ? options['rewrites']
    ...
    if (this._rewrites.length) {
      ...
      this._rewritesNode = {
        id: createUniqueId(),
        label: `Rewrites ${this.enabled ? '' : 'disabled '}(${this._rewrites.length})`,
        children: [],
        parent: treeViewNode,
        tooltip: '',
        uri: uri,
  ...
  matches(msg: DltMsg): boolean {
    ...
    for (let i = 0; i < this._rewrites.length; ++i) {
      if (this._rewrites[i].matchFilter.matches(msg)) {
        return true
      }
    }
  ...
  transformCb(msg: DltMsg) {
    try {
      ...
      if (msg._payloadText) {
        ...
        for (let i = 0; i < this._rewrites.length; ++i) {
          if (i === this._rewrites.length - 1 || this._rewrites[i].matchFilter.matches(msg)) {
            // todo this might evaluate payloadText. Wont lead to recursive call but its a bit undeterministic whether the filter will get the initial or converted value
