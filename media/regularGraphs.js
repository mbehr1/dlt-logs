            ...
            tooltip: {
                ...
                callbacks: {
                    ...
                    label: (item) => { // the lower part per item. Should contain: label name and label value (e.g. thread cpu load: 15)
                        ...
                        return (item.dataset.label || 'no dataset label') + ': ' + (item.formattedValue.startsWith('(') ? item.formattedValue.slice(item.formattedValue.indexOf(',') + 1, item.formattedValue.length - 1).trim() : (item.formattedValue != null ? item.formattedValue : 'no item formatted value'));
                    }
